...
for version in $versions; do
    ...
    if [ "$version" = 'NoReg' ]; then
        for dateNR in $datesNR; do
            python DetJacobian.py "$version" "$dateNR""_f""$count" "1.5" >> times_DetJa_NR.txt
            count=$(($count+1))
        done
    count=0
    elif [ "$version" = 'RigidReg' ]; then
        for dateRR in $datesRR; do
            python DetJacobian.py "$version" "$dateRR""_f""$count" "1.5" >> times_DetJa_RR.txt
            count=$(($count+1))
        done
    count=0
    else
        for dateRAR in $datesRAR; do
            python DetJacobian.py "$version" "$dateRAR""_f""$count" "1.5" >> times_DetJa_RAR.txt
            count=$(($count+1))
        done
    fi
