...
for version in $versions; do
    ...
    if [ "$version" = 'NoReg' ]; then
        for dateNR in $datesNR; do
            python WarpAndDice.py "$version" "$dateNR""_f""$count" "1.5" >> times_WarpAndDice_ANTs_NR_SS.txt
            count=$(($count+1))
        done
    count=0
    elif [ "$version" = 'RigidReg' ]; then
        for dateRR in $datesRR; do
            python WarpAndDice.py "$version" "$dateRR""_f""$count" "1.5" >> times_WarpAndDice_ANTs_RR_SS.txt
            count=$(($count+1))
        done
    count=0
    else
        for dateRAR in $datesRAR; do
            python WarpAndDice.py "$version" "$dateRAR""_f""$count" "1.5" >> times_WarpAndDice_ANTs_RAR_SS.txt
            count=$(($count+1))
        done
    fi
